<script lang="ts">
  ...
  // Determine mode from data - if project exists, it's update mode
  $: isUpdateMode = data.mode === "update";
  $: pageTitle = isUpdateMode ? "Edit Project" : "Create New Project";
  $: saveButtonText = isUpdateMode ? "Update Project" : "Save Project";

  // Use form values if available (after form submission), otherwise use project data for update mode, or empty for create mode
  $: values =
    form?.values ?? (isUpdateMode && data.project ? data.project : {});

  const recommendation = form?.recommendation ?? null;

  // In update mode, allow saving without AI recommendation
  $: hasRecommendation = isUpdateMode || Boolean(recommendation);
...
>
  ...
  {#if form?.saved}
    ...
    >
      Project {form.mode === "update" ? "updated" : "saved"} successfully.
    </div>
  {/if}
</div>
